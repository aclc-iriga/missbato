...
<!-------------------------------------------------------->
    ...
    <table class="table sticky-top table-bordered">
        ...
        <tfoot>
            <tr class="border-0">
                <td class="px-0 pt-2 border-0">
                    <div class="dropdown">
                        <button class="btn btn-outline-secondary btn-sm dropdown-toggle w-100" data-bs-toggle="dropdown" :style="{ 'visibility': hasOnlineJudge ? 'visible' : 'hidden' }">
                            <span class="fw-bold">ALL JUDGES</span>
                        </button>
                        <ul class="dropdown-menu">
                            <li v-for="([eventKey, event], eventIndex) in Object.entries(events)" :key="event.id">
                                <button
                                    ...
                                    @click="$emit('switch-event-all', event)"
                                >
        ...
        computed: {
            ...
            hasOnlineJudge() {
                ...
                for (const judgeKey in this.processedJudges) {
                    online = this.processedJudges[judgeKey].online;
                    if (online) {
                        break;
                    }
                }
